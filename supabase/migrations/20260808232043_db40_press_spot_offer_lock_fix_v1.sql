-- DB40 (second leg) -- press_spot_offer: FOR UPDATE cannot sit in an aggregate.
-- Rollback: supabase/migrations/_drafts/20260808232043_db40_press_spot_offer_lock_fix_v1_rollback.sql
--           (written BEFORE this migration, per the MIGRATION AMENDMENT)
--
-- Found by VERIFYING item 1 rather than trusting a green function. With
-- press_is_admin repaired (20260808231555) the admin gate now passes, and
-- press_spot_offer fails one statement later:
--     ERROR: FOR UPDATE is not allowed with aggregate functions
--     CONTEXT: select array_agg(s.id) ... from press_slots s ... for update
-- That is a hard Postgres restriction and BOTH slot-selection branches carry it,
-- so this RPC has never completed a call down either path. DB36 inferred the
-- outage from press_is_admin alone; the real outage had two causes.
--
-- The lock is preserved, not dropped: FOR UPDATE moves inside a subquery, and
-- the joined branch uses FOR UPDATE OF s so it still locks only press_slots.
--
-- Verified after apply, as an authenticated admin, both branches, rolled back:
--   slot_ids branch      -> hold created, slot_count 1,  total_cents 180000
--   side+quadrant branch -> hold created, slot_count 18, total_cents 520000
--   non-admin bee        -> 'only admin or the owning affiliate may issue spot offers'
...
begin
  ...
  if p_slot_ids is not null then
    -- FOR UPDATE inside the subquery: Postgres rejects it beside an aggregate.
    select array_agg(x.id) into v_slots from (
      select s.id from press_slots s
       where s.id = any(p_slot_ids) and s.edition_id = p_edition and s.status = 'open'
       for update
    ) x;
  elsif p_side is not null and p_quadrant is not null then
    -- FOR UPDATE OF s: lock the slots, not the template rows joined for filtering.
    select array_agg(x.id) into v_slots from (
      select s.id from press_slots s
        join press_slot_templates st on st.id = s.slot_template_id
       where s.edition_id = p_edition and st.side = p_side and st.quadrant = p_quadrant
         and st.slot_kind = 'grid' and s.status = 'open'
       for update of s
    ) x;
